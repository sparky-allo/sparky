				...
				struct aotg_ring *ring, struct aotg_td *td)
{
	...
	num_sgs = urb->num_mapped_sgs;	
	num_trbs = count_urb_need_trbs(urb); /*patch 0020 NA*/
...
//			printk("%s, td still not finish\n", __FUNCTION__);
	...
	if (urb->actual_length > urb->transfer_buffer_length) {
		ACT_HCD_DBG
		urb->actual_length = 0;
		if (td->urb->transfer_flags & URB_SHORT_NOT_OK)  /*patch 0020 NA */
...
//	ep->error_count = 0;      /*patch 0014 */
	...
	//	if (ep->hep->enabled == 0)   /*patch 006 NA */
			break;
	} while (ret == 0);
