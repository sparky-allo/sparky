...
static int aotg_speed = 1;
module_param(aotg_speed, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(aotg_speed, "0 for usb 1.0, 1 for usb 2.0 ");
	...
	if (ep->has_hub) {
		if (urb->dev->speed == USB_SPEED_HIGH) {
			...
			if (urb->dev->speed == USB_SPEED_LOW) {
				...
				printk("SPEED3_1 is %d \n",urb->dev->speed);
			} else {
				writeb(urb->dev->portnum, ep->reg_hcep_port);
				printk("SPEED3_2 is %d \n",urb->dev->speed);
			}
		}
...
//		HUB_DEBUG("irqvector:%d, 0x%x\n", irqvector, irqvector);
		...
		//			acthcd->speed = USB_SPEED_LOW;  // sudeep
		//			if(aotg_speed){
		//				acthcd->speed = USB_SPEED_HIGH;
		...
		//		} else if (readb(acthcd->base + USBCS) & USBCS_LSMODE) {
				if (readb(acthcd->base + USBCS) & USBCS_LSMODE) {
...
void aotg_check_trb_timer(unsigned long data)
{
	...
	acthcd->check_trb_mutex = 1;    /*patch 0021 */
	...
	acthcd->check_trb_mutex = 0; /*patch 0021 */
	spin_unlock_irqrestore(&acthcd->lock, flags);
	return;
}
	...
	if (ep->has_hub) {
		...
		//	printk("SPEED is %d \n",urb->dev->speed);
			...
			if (urb->dev->speed == USB_SPEED_LOW) {
				...
				writeb(urb->dev->portnum, ep->reg_hcep_port);
				printk("SPEED_2 is %d dev-port %d hcep %d\n",urb->dev->speed,urb->dev->portnum,ep->reg_hcep_port);
			}
		}
		writeb(0, ep->reg_hcep_splitcs); //sudeep
	} else {
		writeb(usb_pipedevice(urb->pipe), ep->reg_hcep_dev_addr);
		writeb(urb->dev->portnum, ep->reg_hcep_port);
		printk("SPEED1 is %d \n",urb->dev->speed);
...
//	ep->udev = usb_get_dev(urb->dev);  /*patch 003 */
	...
	case PIPE_INTERRUPT:
		...
		writeb(ep->interval, ep->reg_hcep_interval);		
		printk("TEST1urb->interval: %d\n", urb->interval);
		printk("TEST1urb->ep->desc.bInterval: %d, reg_interval:0x%x\n", 
	...
	//	printk(KERN_DEBUG "iso_packets:%d, bInterval:%d, urb_interval:%d, reg_con:0x%x\n",
		printk("TEST0iso_packets:%d, bInterval:%d, urb_interval:%d, reg_con:0x%x\n",
...
static int aotg_hub_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	/*below condition check is important, "urb not submitted case" urb status check once again to meet the timing 0025*/
//	if (retval!=EBUSY){
//		retval = usb_hcd_check_unlink_urb(hcd, urb, status);
//	}
	if (retval) {
		printk(KERN_WARNING "%s, retval:%d, urb not submitted or unlinked\n", __FUNCTION__,retval);
		printk(KERN_WARNING "hcd: %p,urb :%p,status : %d \n",hcd,urb,status);
//		retval = usb_hcd_check_unlink_urb(hcd, urb, status);
		goto dequeue_out;
	}
...
//	if (!port_host_plug_detect[acthcd->id])
	if (!port_host_plug_detect[pdev->id])
