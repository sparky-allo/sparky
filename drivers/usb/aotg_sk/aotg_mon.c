...
void aotg_uhost_mon_init(struct work_struct *w)
{
	if (port_host_plug_detect[0]) {
		...
		printk("start mon 0 ......\n");
		mod_timer(&aotg_uhost_mon0->hotplug_timer, jiffies + msecs_to_jiffies(1));    /*usb 2.0 boot wakep */
        	if (aotg_udc_enable[0]){
			aotg_udc_register(0);
		}
	}
	if (port_host_plug_detect[1]) {
		...
		printk("start mon 1 ......\n");
		mod_timer(&aotg_uhost_mon1->hotplug_timer, jiffies + msecs_to_jiffies(1));
    		if (aotg_udc_enable[1]){
			aotg_udc_register(1);
		}
	}
