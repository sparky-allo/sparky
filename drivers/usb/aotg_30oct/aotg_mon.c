...
static void aotg_dev_unregister(struct work_struct *w)
{
	...
	unlock_system_sleep(); // patch 0022 sud
	return;
}
...
void aotg_uhost_mon_init(struct work_struct *w)
{
	if (port_host_plug_detect[0]) {
		...
		mod_timer(&aotg_uhost_mon0->hotplug_timer, jiffies + msecs_to_jiffies(100));    /*usb 2.0 boot wakep 1000 chd to 1 */
        	if (aotg_udc_enable[0]){
			aotg_udc_register(0);
		}
	}
	if (port_host_plug_detect[1]) {
		...
		mod_timer(&aotg_uhost_mon1->hotplug_timer, jiffies + msecs_to_jiffies(100));   /*usb 2.0 boot wakep 1000 chgd to 1 */
    		if (aotg_udc_enable[1]){
			aotg_udc_register(1);
		}
	}
