				...
				struct aotg_ring *ring, struct aotg_td *td)
{
	...
	num_sgs = urb->num_mapped_sgs;	
//	num_trbs = count_urb_need_trbs(urb); //patch 0020
...
//		ACT_HCD_DBG
	...
	/*patch 0022 301017 */	
	if (num_trbs > 1){
//		pr_warn("iso urb: %p, is_out: %d, num_trbs: %d\n",urb, is_out, num_trbs);
//	printk(KERN_DEBUG"iso urb: %p, is_out: %d, num_trbs: %d\n",urb, is_out, num_trbs);
		if(!is_room_on_ring(ring, num_trbs))
			printk("ring not free \n");
	}
...
//					index, readw(acthcd->base + HCOUTxBUFEMPTYIRQ0));
			pr_warn("noticd:%s, IN%dIRQ:0x%x; OUT%dIRQ:0x%x, urb: %p\n",
 					__FUNCTION__, index, readw(acthcd->base + HCINxDMAIRQ0),
				index, readw(acthcd->base + HCOUTxBUFEMPTYIRQ0), urb);
			dump_stack();
 ...
 */
	if (urb->actual_length > urb->transfer_buffer_length) {
		ACT_HCD_DBG
		urb->actual_length = 0;
//		if (td->urb->transfer_flags & URB_SHORT_NOT_OK)  /*patch 0020 */
...
//	ep->error_count = 0;      /*patch 0014 */
	...
	if (ring->type == PIPE_ISOCHRONOUS) {
		do {
			...
			if (!td)
				break;
	//		printk("TEST before isoc_finish_td");
	...
	//		if (ring->dequeue_trb != ring->first_trb) /* patch 0018 */
	//			aotg_reorder_iso_td(acthcd, ring);
		...
		do {
			...
			if (!td)
				break;
	//		printk("TEST before intr_finish_td");
...
void aotg_ring_irq_handler(struct aotg_hcd *acthcd)
{
	...
	unsigned long flags;
	bool dma_nest = false; /* patch 0021 */
