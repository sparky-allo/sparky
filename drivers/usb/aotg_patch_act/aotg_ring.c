...
//		if (ring->type == PIPE_BULK) {    /*patch 0022 */
		if (ring->type == PIPE_BULK || ring->type == PIPE_ISOCHRONOUS) {
			token &= ~TRB_CHN;
			if (ring->is_out)
				...
				struct aotg_ring *ring, struct aotg_td *td)
{
	...
	/*patch 3010107 print part comtd*/
//	if (num_trbs > 1){
...
void aotg_ring_irq_handler(struct aotg_hcd *acthcd)
{
	...
	if (acthcd->check_trb_mutex == 1)   /*patch 0021 */
                dma_nest = true;
